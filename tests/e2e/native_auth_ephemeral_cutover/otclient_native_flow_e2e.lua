...
local function tryCharacterLogin()
	...
	if type(G) ~= "table" or type(G.characters) ~= "table" or #G.characters == 0 or type(G.sessionKey) ~= "string" or G.sessionKey == "" or not CharacterList then
	...
	for _, character in ipairs(G.characters) do
		if tostring(character.name) == CHARACTER and tostring(character.worldName) == WORLD and tostring(character.worldIp) == HOST and tonumber(character.worldPort) == GAME_PORT then
			matched = true
			break
		end
	end
	...
	for _, widget in ipairs(characterListWidget:getChildren()) do
		if tostring(widget.characterName) == CHARACTER and tostring(widget.worldName) == WORLD and tostring(widget.worldHost) == HOST and tonumber(widget.worldPort) == GAME_PORT then
			selected = widget
			break
		end
	end
