...
class CleanupCertificationTests(unittest.TestCase):
    ...
    def _write_normal_artifacts(self, gameplay_status: str) -> None:
        (self.artifacts / "result.json").write_text(
            json.dumps({"status": gameplay_status}) + "\n",
            encoding="utf-8",
        )
        ...
        (self.artifacts / "otclient-exit-code.txt").write_text(
            "0\n",
            encoding="utf-8",
        )
        (self.artifacts / "canary.stdout.log").write_text(
            "online\n",
            encoding="utf-8",
        )
        (self.artifacts / "xvfb.log").write_text(
            "started\n",
            encoding="utf-8",
        )
    ...
    @staticmethod
    def _no_op_reaper(pgid, proc_root):
        members = cc._pg_members(pgid, proc_root, set())
        return {
            "pgid": pgid,
    ...
    def _certify(
        ...
        *,
        ...
        return cc.certify(
            ...
            group_reaper=group_reaper or self._no_op_reaper,
        )
    ...
    def test_baseline_uses_logical_paths_only(self):
        baseline = json.loads(
            (self.artifacts / cc.BASELINE_NAME).read_text(encoding="utf-8")
        )
        rendered = json.dumps(baseline)
        self.assertNotIn(str(self.root), rendered)
        self.assertEqual(
            "otclient",
            baseline["files"]["otclient_init"]["root"],
        )
        self.assertEqual(
            "init.lua",
            baseline["files"]["otclient_init"]["path"],
        )

    def test_invalid_pid_is_never_killed_and_fails_closed(self):
        (self.artifacts / "canary.pid").write_text(
            "not-a-pid\n",
            encoding="utf-8",
        )
    ...
    def test_outside_process_group_is_untouched(self):
        ...
        with mock.patch.object(cc.os, "killpg") as killpg:
            report = self._certify(
                group_reaper=lambda pgid, root: cc.reap_process_group(
                    pgid,
                    root,
                    killpg=killpg,
                    sleep=lambda _: None,
        ...
        check = next(
            ...
            if item["id"] == "runner_process_group_empty"
        )
        self.assertEqual("pass", check["status"])
        self.assertTrue((self.proc / "500").exists())

    def test_owned_process_group_residue_is_reaped(self):
        ...
        def killpg(pgid, sig):
            ...
            if sig == signal.SIGTERM:
                for entry in list(self.proc.iterdir()):
                    if entry.name.isdigit():
                        entry.joinpath("stat").unlink()
                        entry.rmdir()
        ...
        self.assertEqual([(90, signal.SIGTERM)], calls)
        self.assertEqual(
            [501],
            report["process_group_cleanup"]["members_before"],
        )
        self.assertEqual(
            [],
            report["process_group_cleanup"]["members_after"],
        )

    def test_unreapable_runner_process_group_residue_fails(self):
        self._add_proc(501, 90)
        report = self._certify()
        self.assertFalse(report["cleanup_certified"])
        check = next(
    ...
    def test_missing_client_after_client_activity_fails(self):
        ...
        check = next(
            item
            for item in report["checks"]
            if item["id"] == "process_stopped:primary_client"
        )
        self.assertFalse(report["cleanup_certified"])
    ...
    def test_players_online_rows_fail_cleanup(self):
        def runner(command, query, env):
            return (
                (0, "1", "")
                if query == "SELECT COUNT(*) FROM players_online"
                else (0, "0", "")
            )
    ...
    def test_leftover_fault_marker_fails(self):
        (self.repo / ".agent-e2e-fault-injection").write_text(
            "leftover\n",
            encoding="utf-8",
        )
        report = self._certify()
    ...
    def test_declared_secondary_requires_stop_and_exit_evidence(self):
        ...
        self.assertEqual(
            ...
            next(
                item
                for item in report["checks"]
                if item["id"] == "secondary_client_stopped:secondary"
            )["status"],
        )
    ...
    def test_workspace_change_fails_and_partial_is_explicit(self):
        (self.otclient / "init.lua").write_text(
            "changed\n",
            encoding="utf-8",
        )
        report = self._certify()
