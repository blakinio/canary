...
The runtime hooks collect raw/final experience, outgoing/incoming damage, effective healing and overhealing, negative mana changes, kills, deaths, vocation/level context, party/shared-experience context, damage types, combat duration and session duration.
...
Sessions are created lazily on the first recorded metric. Merely logging in does not create an empty database row.
...
Before recording the first metric on a later UTC date, the runtime closes the previous session with the reason `utc-day-rollover` and creates a new session. A short rollover fragment is retained even below `minimumSessionSeconds`. This keeps post-midnight events out of the previous day's aggregate.

### Deaths shorter than the normal minimum

**Short death sessions** are retained even when they last less than `minimumSessionSeconds`. The ordinary minimum still removes short, non-death noise, but it cannot suppress a real death from balance statistics.
...
The `mana_spent` aggregate represents all negative mana changes observed by the mana-change event. Depending on server mechanics, this can include mana shield damage. Mana-only sessions are not persisted as hunts unless combat or a death also occurs.
...
After `maxRetryAttempts` retries, the session leaves the normal queue and enters a bounded in-memory dead-letter queue. The runtime then tries to upsert a compact terminal failure record into `analytics_dead_letters`. Dead-letter writes are idempotent by `session_uuid`.

Persisted dead-letter rows are investigation history, not an automatic database replay queue. `/analytics deadletters` retries persistence of records still waiting in the process-local dead-letter queue; it does not replay rows already stored in MariaDB.

A database outage therefore does not block gameplay. Normal sessions remain bounded by `queueLimit`, dead letters remain bounded by `deadLetterQueueLimit`, and overflows are logged and counted.

## Schema compatibility

The runtime checks `analytics_schema_migrations` before starting database-backed collection. Analytics remains stopped when the migration table is missing or too old. The game server continues to run normally.
...
Applied migration files are protected by SHA-256 checksums. Add a new numbered migration instead of editing an applied migration.
...
`recordSpell` stores spell mana in the spell aggregate but does not add it again to session-wide `mana_spent`.

Supply and loot remain disabled until enabled. Integrated runes record supply cost only when `configKeys.REMOVE_RUNE_CHARGES` is enabled, matching `REMOVE_RUNE_CHARGES`. Infinite-charge rune configurations do not create fictional expenses.
...
`/analytics flush` forces normal and delayed retry entries. `/analytics deadletters` retries persistence of the process-local dead-letter queue. `/analytics schema` rechecks database compatibility. Runtime enable/disable resets after restart.
...
- `successfulFlushes`, `failedFlushes`, `persistedSessions`, `retriedSessions` and `persistedDeadLetters`;
- `lastFlushDurationMs`, `lastFlushProcessed`, `lastFlushFailed` and `oldestQueuedAgeSeconds`;
- batching, context and correctness lifecycle counters.
...
After validating schema readiness, CPU, memory, queue depth, retry counters, lifecycle counters and data quality, enable spell, supply and loot reporting. Use a new `CANARY_SERVER_VERSION` when deploying changed collection or aggregation semantics.
...
- Ordinary short sessions are filtered, but deaths and UTC rollover fragments are retained.
- Staff, PvP and player-summon combat are excluded by default.
- Schema incompatibility disables Analytics, not Canary.
- Queues, batches, retries and dead letters remain bounded.
- Database-disabled mode does not accumulate an undrainable queue.
- Session, detail and dead-letter writes are idempotent.
- Database failures do not stop the game server.
- Player names can be omitted.
