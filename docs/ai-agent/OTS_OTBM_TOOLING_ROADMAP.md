...
| 7 | Geometry and consistency audit | active draft | #320 |
...
Validated implementation head `69658ddabf74b4202ef7ee896e2262c67d7224a8`:
...
The first dedicated runs exposed only the known synthetic fixture `maxItemDepth=-1` versus binary `0` edge for maps without node-items. Fixtures were repaired by retaining one neutral known nested item; no production validation was weakened. Subsequent heads additionally confine render-manifest output, use bounded area-posting iteration, preserve current-main catalogue content and update programme documentation. Their own current-head and ready-state checks remain required before merge.
...
Phases 1–6 are merged and archived. Phase 7 is active in task `CAN-20260714-otbm-geometry-consistency-audit`, branch `feat/otbm-geometry-consistency-audit` and draft PR #320.

Do not reopen historical OTBM PRs or continue their branches. Do not combine Phase 7 with Harlow cleanup, `0,0,0` teleport repair, Bone Capsule repair, Targuna donor-map remediation, map writing, gameplay fixes or Phase 8. Phase 8 may start only after #320 and its separate lifecycle cleanup are merged and a fresh current-main/ownership preflight is complete.
