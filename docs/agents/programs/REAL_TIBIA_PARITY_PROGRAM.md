...
| OTBM Tibia client reference | `otbm-tooling` module record, `docs/agents/programs/OTBM_TIBIA_CLIENT_REFERENCE_PROGRAM.md` and `docs/ai-agent/OTBM_TIBIA_CLIENT_REFERENCE_ARCHITECTURE.md` | Treat exact user-supplied client packages as provenance-pinned read-only reference evidence. Reuse the canonical World Index, appearances/assets indexes, Script Resolution, Reachability, factual renderer, mutation/finalization paths and Universal Physical E2E; never infer client build or `staticmapdata.object_id` equivalence. |
