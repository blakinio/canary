...
Before substantial implementation, product-facing validation, audit, E2E, PR cleanup, or task closeout, read and follow `DELIVERY_COMPLETENESS_AND_CLOSEOUT.md`. It is mandatory for prompt evaluation discipline, trust and authority boundaries, delivery classification, frontend/backend or producer/consumer completeness, independent audit, real E2E, exact-head validation, terminal PR states, and archival. A worker summary is not terminal evidence.
...
- Legacy flat `owned_paths` lists are indexed as `legacy_exclusive` and produce migration warnings. They are enforced only with `--strict-legacy` until old records are migrated.
...
- select at most one additional task after the terminal entry task and only after repeating ownership and overlap preflight and satisfying the remaining-budget and no-stall requirements in `ANTI_STALL_AND_EXECUTION_BUDGET.md`.
...
- Backend or API checks do not replace real client or frontend E2E for user-facing work.
...
4. treat the task record and Git or PR state as durable and the worker session as disposable;
...
11. before `completed`, verify independent audit PASS, required E2E PASS or NOT_APPLICABLE with reason, exact-head required CI PASS, zero unresolved review threads, zero unintentionally open related PRs, terminal task state, and released ownership.
