...
These rules apply to `docs/agents/**` and supplement the root `AGENTS.md`.
...
- `exclusive` claims are advisory locks enforced by deterministic validation. Two active tasks must not hold overlapping exclusive claims.
- `shared` claims identify narrow coordinated edits to catalogues, contracts, changelogs, or indexes. Refresh from current `main` and preserve other agents' entries.
- `read_only` claims identify dependencies that the task may inspect but must not edit.
- Legacy flat `owned_paths` lists are treated as exclusive until migrated.
- Broad claims such as `src/**`, `data-otservbr-global/**`, or `docs/**` require a documented reason and should normally be replaced by exact paths or bounded globs.
