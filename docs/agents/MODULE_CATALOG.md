...
| Imbuement validation audit | active (#166) | Deterministic read-only audit of Imbuing XML structure, current reference values and cross-file runtime wiring; emits JSON findings and a runtime scenario plan | `tools/ai-agent/imbuement_validation.py`, `test_imbuement_validation.py`, `.github/workflows/imbuement-validation.yml`, `docs/ai-agent/IMBUEMENT_VALIDATION_REPORT.md`, `IMBUEMENT_RUNTIME_TEST_PLAN.json` | Reuse for future Imbuing drift checks. Findings are evidence, not authorization for automatic gameplay or economy changes. |
