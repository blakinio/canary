...
| Gameplay Analytics | active correctness hardening (#135); operator enablement remains manual | Runtime telemetry with UTC rollover/combat eligibility, bounded persistence, configurable maintenance brackets and dimension-safe reporting | `data-otservbr-global/scripts/lib/gameplay_analytics_*.lua`, `tools/analytics/**`, reporting schema/dashboard, workflows/docs | Reuse the live `GameplayAnalytics` wrapper stack; load correctness after reliability, never reload the core, and configure brackets through maintenance `LEVEL_BRACKETS`. |
