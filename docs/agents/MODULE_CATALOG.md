...
| Weapon Proficiency achievement audit | active (#195) | Read-only evidence for mastery achievements 564–567, runtime transition/backfill paths and hashed asset/server secret-build validation | `tools/ai-agent/weapon_proficiency_*`, focused tests/workflow, report, runtime plan and metadata-only baseline | Reuse before adding mastery hooks or ID 567. Require exact item identity plus a valid active proficiency definition; never commit the source asset archive. |
