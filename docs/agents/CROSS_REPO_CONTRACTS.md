...
| `OTS-20260728-game-catalog-v1` | exporter PR #991; schema 1.1 PR #1006; endpoint PR #1010; schema 1.2 task `CAN-20260729-game-catalog-loot-threshold-schema` | Platform importer PR #272; schema 1.1 PR #299; schema 1.2 PR #310 | Offline file contract `oteryn.game-catalog`; immutable schemas `1.0.0`/`1.1.0`; schema `1.2.0` SHA-256 `a9fa1e3c6366a90d61005796511c344ced9c39594ed676276279a5917287c6de` adds `canary_dynamic_threshold_v1` with exact configured threshold and roll maximum | schema 1.2 consumer-first-safe; older consumers fail closed; all snapshots remain inactive until a separate activation gate | schema 1.2 consumer merged; producer active | Platform #310 merged as `2a97d0a04f1d6ecc02f4ec52b8aba1839a0ac77b`; Canary #1010 merged as `24ce121f487f711cc19214f59ac0fb21d80ff737`; producer task must pin byte-identical schema/fixture and pass full default-datapack runtime proof |
...
- **Schema 1.1 unknown boundary:** `verified_content_through_release=null` means the datapack-wide boundary is unproven. It permits inactive export/import review only; Platform activation fails closed. Schema `1.0.0` remains byte unchanged.
...
- **Schema 1.1 rollout:** merge/deploy dual-schema Platform support first, then Canary producer support. Retained `1.0.0` snapshots remain compatible. Do not route `1.1.0` output to an older consumer.
- **Schema 1.2 loot model:** `canary_dynamic_threshold_v1` carries exact configured `chance_threshold` and declared `roll_maximum`; it is contextual runtime configuration, not a context-free percentage. Schemas 1.0/1.1 retain rational chance fields byte-for-byte.
- **Schema 1.2 rollout:** Platform PR #310 is merged consumer first. Canary producer support follows in `CAN-20260729-game-catalog-loot-threshold-schema`; no import or activation is authorized by either merge.
