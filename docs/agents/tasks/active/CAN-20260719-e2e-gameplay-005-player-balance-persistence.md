...
- [ ] Add `player_balance` checks with exact `equals` value.
- [ ] Validate `equals` as a strict integer in the current unsigned BIGINT range `0..18446744073709551615`.
- [ ] Compile only one fixed-shape semicolon-free scalar SQL equality query against `players.balance` by exact fixture character name.
- [ ] Do not expose caller-controlled table names, columns, predicates or SQL fragments.
- [ ] Keep `player_field` as the only current phase-two controlled-client typed persistence check; bank balance remains database-only.
- [ ] Add focused validation/compiler and mixed-contract tests.
- [ ] Document the database-only bank-balance boundary and feature-owned stronger assertions.
- [ ] Keep feature-specific expected balances and economy actions out of shared fixtures.
...
proven:
  ...
  - no feature-specific expected bank balance is required in shared platform fixtures
  - player_field remains the only trustworthy generic controlled-client typed persistence check
  - PR 586 exact final head passed CI, Universal Agent E2E, Agent Task Ownership and autofix before expected-head squash merge
derived:
  - exact unsigned-64 equality against players.balance is a bounded typed database assertion for durable bank/economy state
  - database-only treatment avoids claiming a generic maintained-client bank balance API that was not found
unknown:
