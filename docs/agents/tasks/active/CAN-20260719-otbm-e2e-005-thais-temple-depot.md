...
- [ ] Require PR #599 landmark binding to be merged and rebase/update this branch from the resulting current `main` before final route generation.
...
- [ ] Add only feature-owned deterministic route request/fixture/assertion data unless a fresh, proven reusable integration-contract gap requires a shared-path change.
...
proven:
  ...
  - current Universal Agent E2E workflow resolves the scenario before any route artifact generation or download step, so a reusable route-artifact preparation bridge must be proven available or supplied without duplicating the runner/workflow
blockers:
  ...
  - current inspected Universal Agent E2E path does not itself materialize the required canonical route JSON before scenario resolution; the narrowest reusable integration path must be established before adding the physical scenario
first_failure:
  marker: none
  evidence: No validation failure has been observed on this task branch yet; the first commit only creates the ownership/task boundary.
conflicts: []
rejected_hypotheses:
...
validation:
  ...
  ci: pending
next_action: Open the draft PR for ownership visibility, then determine the narrowest existing/reusable route-artifact preparation boundary that can generate and preflight the semantic route before the unchanged Universal physical lifecycle consumes follow_route; do not modify PR #599 owned paths and do not finalize until #599 is merged.
```
