...
The architecture must cover at minimum:
...
- [ ] Add a durable programme with bounded queue entries and exact next action.
- [ ] Add a technical architecture defining contracts, provenance, joins, failure states and non-goals.
- [ ] Update the OTBM roadmap to route future official-client reference work through the new programme.
- [ ] Update the module catalogue and Real Tibia source guidance for this reusable integration point.
- [ ] Update the OTBM real-Tibia registry record without creating a duplicate tooling module.
- [ ] Update the global Real Tibia parity programme discovery surface.
- [ ] Update the agent-facing changelog.
- [ ] Ensure the plan does not authorize raw client files, `.otbm`, `.widx`, `items.otb` or generated large artifacts in Git.
- [ ] Ensure `staticmapdata` is treated as bounded house-layout/reference evidence, not a full OTBM source or automatic map generator.
- [ ] Ensure minimap evidence cannot replace canonical World Index/Reachability mechanics evidence.
- [ ] Ensure all proposed mutation/adoption routes require existing review/approval and post-mutation validation contracts.
...
- Open PR #759 owns only QA-006/QA-007 certification/assurance implementation paths and does not overlap this task's planned documentation paths.
...
PROVEN:
- Branch `docs/otbm-tibia-client-reference-20260723` starts from main `8837f35eb43da6a3ed7efc6a1e8f3bca19342d2e`.
- No changed-path overlap with open OTBM QA PR #759.
- Architecture source inventory is pinned to `beats-dh/Beats-Assets-Editor@ed827be34c279d1279ad3dde3af434b148ac05c7`.
- This task is docs-only and must not add parser/runtime/map implementation.

NEXT_ACTION:
- Publish an early draft PR, then add the programme, technical architecture and narrow shared-document discovery updates listed in `owned_paths`.
