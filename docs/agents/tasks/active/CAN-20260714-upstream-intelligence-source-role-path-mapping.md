...
updated: 2026-07-14T16:42:00+02:00
...
`map_candidate()` currently calls `Registry.matched_modules(path)` without source context. `matched_modules()` searches every bucket (`server`, `client`, `data`, `tests`, `docs`). Therefore a server path such as `src/canary_server.cpp` can match the broad client pattern `src/**` in `protocol.yaml`.

PR #335 removed the incorrect test expectation but intentionally did not change the mapper or `protocol.yaml`.

# Selected design

Extend the existing Upstream Intelligence source registry with one explicit policy per watched source:

```text
module_mapping.path_buckets
```

The existing mapper will receive the source record and keep only matches from those buckets. No repository-name checks will be hardcoded in mapping code.
...
- Remere's Map Editor is map/data tooling whose implementation root uses `source/**`; its source policy will not be treated as maintained-client code.
- Client Editor patches/repackages Tibia client artifacts and appearances data; it receives a separate explicitly client-capable policy.

# Acceptance criteria

- [ ] Existing source registry is the only mapping-policy registry.
- [ ] Every configured source declares allowed path buckets explicitly.
- [ ] Server roles cannot consume client buckets.
- [ ] Client role cannot consume server buckets.
- [ ] Remere's Map Editor and Client Editor have separately justified policies.
- [ ] Unknown/unsupported role maps conservatively to no modules and explicit unmapped paths.
- [ ] Existing mapper remains discovery-only and deterministic.
- [ ] `triage_status: needs-triage` and `decision_state` remain unchanged by mapping.
- [ ] One path can still map to several modules inside allowed buckets.
- [ ] Focused tests assert both valid matches and absence of invalid matches.
- [ ] Source schema and domain validation reject unsafe role/bucket combinations.
...
- [ ] No module, `protocol.yaml`, gameplay, runtime, protocol implementation, client, DB, map, OTBM, datapack, asset, watcher or E2E change.
