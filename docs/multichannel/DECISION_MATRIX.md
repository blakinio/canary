...
| 2.11 | One online character per account, cluster-wide, defense in depth | `cluster_sessions` DB table (`PRIMARY KEY(account_id)` + `UNIQUE(player_id)`) + Redis lease/fencing; `ClusterRuntime` wires both into real login (acquire)/logout (release)/heartbeat (renew, outage handling) via `IClusterSessionRepository`/`DbClusterSessionRepository`, verified against a real MariaDB (a real upsert-corruption edge case was found and fixed - see TEST_PLAN.md) | ✅ schema+algorithm+Redis+DB engine hooks / 📐 admin tool for orphaned `DIRTY` sessions still not implemented - migration plan in `CROSS_PROCESS_DB_ROW_HANDOFF.md` §12 (build on the same handoff primitive, not a bespoke fix) |
| 2.12 | Mail/parcel exactly-once across channels | **Fixed** (#308, #343): a real cross-channel **loss** race (not just a missing idempotency guard) was found while scoping GM commands in an earlier phase - `Mailbox::sendItem` wrote to a throwaway offline copy of the recipient when they were actually online on a different channel, which that channel's own next save silently overwrote. Root-caused (`IOLoginDataSave::savePlayerInbox`/`savePlayerItem` do a full delete+reinsert from live memory, not a merge - a direct DB write can never survive the owning process's next save) and fixed via a general-purpose cross-process DB-row handoff mechanism (`cluster_pending_operations` table + `ClusterRecordHandoff` orchestrator, see `CROSS_PROCESS_DB_ROW_HANDOFF.md`), not a mail-specific patch: `sendItem` now durably enqueues a `DELIVER_MAIL_ITEM` operation and either applies it synchronously (when safe) or leaves it for the confirmed owner's own sweep - never a direct write to a possibly-stale copy | ✅ fixed and tested (25/25 real gtest across both PRs; SQL hand-verified against real MariaDB incl. a genuine two-connection row-lock contention test) |
