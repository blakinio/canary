...
| 2.3 | Market operations transactional + idempotent | `economic_ledger.transaction_uuid` PK contract; wired into `IOMarket::processExpiredOffers` (the expiry background job), verified against a real MariaDB incl. the duplicate-`transaction_uuid` rejection | ✅ expiry job / 📐 the 3 live call sites (create/cancel/accept offer) |
...
| 8.1–8.4 | Transactions, idempotency, ledger, item UID | `economic_ledger` schema, wired into the market-offer-expiry job (`IOMarket::processExpiredOffers`) via `EconomicLedgerStore`; item UID explicitly *not* added (see rationale below) | ✅ expiry job / 📐 remaining call sites (live market ops, mail, house) / 🔭 item UID |
...
| 11 | Leader election for singleton jobs | `EventClusterScope` enum + job inventory table in OPERATIONS.md; `ClusterLeaderElection` primitive (reuses `ClusterSessionManager`'s Redis lease/fencing mechanism, keyed by job name) + `ClusterJobLeadershipRegistry` cache, wired into `IOMarket::checkExpiredOffers` (recurring job) and `Game::loadBoostedCreature`/`IOBosstiary::loadBoostedBoss` (one-shot startup jobs, a one-shot leader-election race rather than a sustained check). House rent charging/auction settlement, previously misclassified here as cluster-singleton, corrected to `per-channel` after tracing that houses are already disjoint per channel — see OPERATIONS.md | ✅ primitive+tests+3 jobs wired / 📐 remaining genuinely-global jobs unwired |
| 12 | Status/admin/metrics | Documented command and metric list, tagged by channel/instance; `Game.getPlayerClusterChannel`, `Game.getClusterOnlinePlayers`, `Game.getPlayerChannelSwitchHistory` implemented (all read-only, `cluster_sessions`/`channel_switch_audit` DB lookups) | ✅ three GM commands / 🔭 the rest + metrics |
