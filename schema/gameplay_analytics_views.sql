...
-- dashboard in grafana/gameplay-analytics-dashboard.json.
...
SELECT
    ...
    ROUND(LEAST(`shared_experience_seconds`, `combat_seconds`) / NULLIF(`combat_seconds`, 0) * 100, 2) AS `shared_experience_percent`
FROM `analytics_daily_balance`;

-- Solo and party are aggregated separately before this view. Never infer the
-- mode from an average after unlike sessions have already been merged.
...
SELECT
    ...
    ROUND(`experience_raw` / NULLIF(`combat_seconds`, 0) * 3600) AS `exp_per_hour`,
    ROUND((`loot_value_npc` - `supplies_value`) / NULLIF(`combat_seconds`, 0) * 3600) AS `npc_profit_per_hour`
FROM `analytics_daily_party_mode`;
