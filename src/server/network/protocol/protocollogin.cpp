...
void ProtocolLogin::getCharacterList(const std::string &accountDescriptor, const std::string &password) const {
	...
	if (characterListLayout == AccountCharacterListLayout::LegacyCharacterList) {
		if (g_configManager().getBoolean(MULTICHANNEL_ENABLED)) {
			...
			if (channels.empty()) {
				disconnectClient("No game channel is currently available.");
				return;
			}
			...
			for (const auto &channel : channels) {
				...
				if (channelIp == 0) {
					g_logger().warn("[ProtocolLogin::getCharacterList] - Channel '{}' has a non-numeric external_host '{}'; legacy 8.60 clients require a numeric IPv4 address, skipping this channel.", channel.name, channel.externalHost);
					continue;
				}
				endpoints.push_back({ channel.name, channelIp, static_cast<uint16_t>(channel.gamePort) });
			}
			if (endpoints.empty()) {
				disconnectClient("No legacy-compatible game channel is currently available.");
				return;
			}
...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage &msg) {
	...
	BanInfo banInfo;
	const auto curConnection = getConnection();
	if (!curConnection) {
		return;
	}
	...
	const std::string accountDescriptor = msg.getString();
	if (accountDescriptor.empty()) {
		disconnectClient(fmt::format("Invalid {}.", oldProtocol ? "username" : "email"));
		return;
	}
	...
	if (accountDescriptor == "@livestream") {
		...
		g_dispatcher().addEvent(
			[self = std::static_pointer_cast<ProtocolLogin>(shared_from_this()), password] { self->getLivestreamCharacterList(password); },
	...
	g_dispatcher().addEvent(
		[self = std::static_pointer_cast<ProtocolLogin>(shared_from_this()), accountDescriptor, password] { self->getCharacterList(accountDescriptor, password); },
		__FUNCTION__
	);
}
...
void ProtocolLogin::getLivestreamCharacterList(const std::string &password) const {
	...
	for (size_t index = 0; index < casterCount; ++index) {
		output->addByte(0x00);
		output->addString(casters[index]->getName());
	}
	output->addByte(0x00);
	output->addByte(0x00);
