...
from otbm_tile_insertion_materializer import APPROVAL_FORMAT, RESULT_FORMAT, TileInsertionMaterializerError, materialize_tile_insertions
...
class TileInsertionMaterializerTests(unittest.TestCase):
    ...
    @staticmethod
    def support_area() -> dict[str, object]:
        return {"baseX": 512, "baseY": 512, "z": 7, "tiles": [{"x": 512, "y": 600, "ground": 100, "items": [{"id": 250}]}]}
    ...
    def build_indexes(self) -> None:
        for source, index, manifest in (
            (self.current, self.current_index, self.current_manifest),
            (self.donor, self.donor_index, self.donor_manifest),
        ):
            build_world_index(map_path=source, scanner=self.scanner, output=index, manifest_output=manifest)

    def write_approval(self, positions: list[tuple[int, int, int]]) -> dict[str, object]:
        workspace = self.root / "scan"
        workspace.mkdir(exist_ok=True)
        areas = _area_index(scan_tile_area_spans(map_path=self.current, scanner=self.scanner, workspace=workspace, role="current"))
        donor_report = scan_tile_spans(map_path=self.donor, scanner=self.scanner, workspace=workspace, role="donor")
        donor_spans = {(int(e["x"]), int(e["y"]), int(e["z"])): e for e in donor_report["tiles"]}
        ...
        with WorldIndex(self.donor_index) as donor_index:
            for position in positions:
                ...
                donor_span = donor_spans[position]
                canonical, _ = _canonical_tile(donor_index, position)
                selections.append({
                    "position": list(position),
                    "areaKey": list(area),
                    "currentArea": {"rawSha256": area_span["sha256"], "rawByteLength": area_span["byteLength"]},
                    "donor": {
        ...
        approval: dict[str, object] = {
            ...
            "provenance": {
                "current": {"sourceMap": pin(self.current), "worldIndex": pin(self.current_index), "manifest": pin(self.current_manifest)},
                "donor": {"sourceMap": pin(self.donor), "worldIndex": pin(self.donor_index), "manifest": pin(self.donor_manifest)},
            },
    ...
    def test_01_inserts_missing_tile_and_preserves_complete_current_byte_sequence(self) -> None:
        ...
        make_area_map(self.donor, [self.area([self.tile(300, 999), self.tile(301, 201)])])
        ...
        with WorldIndex(self.current_index) as current_index, WorldIndex(self.donor_index) as donor_index, WorldIndex(self.root / "evidence" / "output.widx") as output_index:
    ...
    def test_02_inserts_multiple_tiles_into_empty_existing_area_in_deterministic_position_order(self) -> None:
        make_area_map(self.current, [self.area([]), self.support_area()])
        make_area_map(self.donor, [self.area([self.tile(302, 202), self.tile(300, 200)])])
        self.build_indexes()
        self.write_approval([(302, 600, 7), (300, 600, 7)])
        spans = self.run_materializer()["rawConfinement"]["outputInsertedSpans"]
        ...
        with WorldIndex(self.root / "evidence" / "output.widx") as output_index:
            ...
            self.assertIsNotNone(output_index.find_tile((512, 600, 7)))

    def test_03_rejects_position_that_already_exists_in_current(self) -> None:
        make_area_map(self.current, [self.area([self.tile(300, 200)])])
        make_area_map(self.donor, [self.area([self.tile(300, 210)])])
        self.build_indexes()
