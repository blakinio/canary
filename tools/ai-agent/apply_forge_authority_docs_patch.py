#!/usr/bin/env python3
"""Update durable Forge authority evidence after verified PR #250 CI."""
...
catalog = "docs/agents/MODULE_CATALOG.md"
replace_once(
    catalog,
    "| Forge history ID resolution | `src/creatures/players/player.hpp`, `src/creatures/players/player.cpp`, `src/database/database_tasks.cpp`, `tests/integration/game/forge_it.cpp`, `tests/unit/players/forge_history_test.cpp` | runtime history model, in-memory pending history, description rendering, async DB persistence, regression tests | enabled when Forge history is registered | Forge history carries stable item IDs for first and second item; description rendering resolves names from IDs with fallback; persistence writes the stored IDs directly; coverage exists in focused unit and integration tests | stale/missing item metadata uses a conservative name fallback; persistence remains asynchronous | #110 / active |",
    "| Forge history ID resolution | `src/creatures/players/player.hpp`, `src/creatures/players/player.cpp`, `src/database/database_tasks.cpp`, `tests/integration/game/forge_it.cpp`, `tests/unit/players/forge_history_test.cpp` | runtime history model, in-memory pending history, description rendering, async DB persistence, regression tests | enabled when Forge history is registered | Forge history carries stable item IDs for first and second item; description rendering resolves names from IDs with fallback; persistence writes the stored IDs directly; coverage exists in focused unit and integration tests | stale/missing item metadata uses a conservative name fallback; persistence remains asynchronous | #110 / active |\n| Forge server authority policies | `src/game/functions/forge_fusion_policy.hpp`, `src/game/functions/forge_transfer_policy.hpp`, `src/creatures/players/player.cpp`, `tests/unit/players/forge_test.cpp`, `tests/integration/game/forge_it.cpp` | regular/Convergence Fusion pair validation and Transfer class/tier validation before mutation | called from Player Forge handlers after item resolution | normal Fusion requires identical item IDs; Convergence Fusion requires different class-4 items in the same normalized slot; Convergence Transfer requires matching class 4 while normal Transfer preserves PR #89 rules | gameplay/E2E proof remains separate; transaction rollback is tracked by F-020/F-021 | #250 / active pending merge |",
)
