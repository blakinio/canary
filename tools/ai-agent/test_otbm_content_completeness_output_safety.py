...
from otbm_content_completeness import MANIFEST_FORMAT, ContentCompletenessError
from otbm_content_completeness_tool import _load_stable_json, _prepare_output, _write_json


class ContentCompletenessOutputSafetyTests(unittest.TestCase):
    def test_rejects_input_symlink(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ContentCompletenessError, "symlink"):
    ...
    def test_rejects_duplicate_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ContentCompletenessError, "distinct"):
    ...
    def test_rejects_output_input_collision(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source = root / "input.json"
            source.write_text("{}", encoding="utf-8")
            with self.assertRaisesRegex(ContentCompletenessError, "one of the input"):
    ...
    def test_no_clobber_then_atomic_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ContentCompletenessError, "already exists"):
