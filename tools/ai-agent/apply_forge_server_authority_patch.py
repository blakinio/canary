...
def marked(text: str) -> str:
    """Remove one visual marker per line without changing tabs/spaces after it."""
    lines = text.strip("\n").splitlines()
    result: list[str] = []
    for line in lines:
        if not line.startswith("|"):
            raise RuntimeError(f"Marked block line is missing '|': {line!r}")
...
print("Patching Player includes")
replace_once(
...
print("Patching Fusion authority")
fusion_anchor = marked(
    """
...
fusion_authority = fusion_anchor + "\n" + marked(
    """
...
print("Patching Transfer authority")
replace_once(
    "src/creatures/players/player.cpp",
...
print("Patching policy unit tests")
replace_once(
...
unit_path.write_text(
    unit_path.read_text(encoding="utf-8").rstrip() + "\n\n" + unit_append,
    encoding="utf-8",
)

print("Replacing invalid normal Fusion success regression")
different_id_success = marked(
    """
...
print("Appending Convergence no-mutation regressions")
integration_append = marked(
    """
