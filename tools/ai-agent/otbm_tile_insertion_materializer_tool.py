...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Insert reviewed complete raw OTBM tile subtrees at missing exact same coordinates "
            "inside already-existing TILE_AREA parents in a create-new map copy."
        )
    )
...
def main() -> int:
    ...
    try:
        result = materialize_tile_insertions(
