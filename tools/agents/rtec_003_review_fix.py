...
''', "test source boundaries")
    text = once(text, '''    def test_all_owner_routes_reach_consumed_with_matching_evidence(self) -> None:
''', '''    def test_owner_metadata_must_match_history_actor(self) -> None:
...
''', "identity tests")
    text = once(text, '''    def test_dry_run_write_and_rollback_are_transactional(self) -> None:
''', '''    def test_consumption_preserves_retained_proof_boundaries(self) -> None:
...
''', "boundary and fence tests")
    path.write_text(text, encoding="utf-8")


def patch_docs() -> None:
    path = ROOT / "docs/agents/real-tibia/evidence/OWNER_REQUEST_LIFECYCLE.md"
    text = path.read_text(encoding="utf-8")
...
- introduce no owner `proves` claim absent from the retained result;
...
def cleanup() -> None:
    path = ROOT / "tools/agents/rtec_003_review_fix.py"
    if path.exists():
        path.unlink()
