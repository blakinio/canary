...
Mutation = Callable[[dict[str, Any], Corpus], dict[str, Any]]
...
def parse_stable_result_ref(value: str) -> StableResultRef:
    ...
    patterns = (
        (rf"github-pr:({REPOSITORY_RE})#([1-9][0-9]*)", "github-pr"),
        (rf"github-commit:({REPOSITORY_RE})@([0-9a-f]{{40}})", "github-commit"),
        (rf"github-actions-run:({REPOSITORY_RE})#([1-9][0-9]*)", "github-actions-run"),
        (rf"github-actions-job:({REPOSITORY_RE})#([1-9][0-9]*)", "github-actions-job"),
    )
    for pattern, kind in patterns:
        ...
        if kind == "github-commit":
            return StableResultRef(value, kind, repository=match.group(1), commit_sha=match.group(2))
        return StableResultRef(value, kind, repository=match.group(1), numeric_id=int(match.group(2)))
    ...
    if match:
        ...
        if not safe_repo_path(repository_path):
            raise RequestLifecycleError(
                f"repo-file reference contains unsafe repository path {repository_path!r}"
            )
        return StableResultRef(
            value,
            "repo-file",
...
def _stable_refs(values: Sequence[str], label: str) -> tuple[list[str], list[StableResultRef]]:
...
def _request_document(corpus: Corpus, request_id: str) -> LoadedDocument:
    matches = [
        ...
        if document.value.get("request_id") == request_id
    ]
    if len(matches) != 1:
        qualifier = "does not exist" if not matches else "is duplicated"
        raise RequestLifecycleError(f"owner request {request_id!r} {qualifier}")
    return matches[0]


def _candidate_corpus(
    corpus: Corpus, document: LoadedDocument, value: dict[str, Any]
) -> Corpus:
    ...
    requests = tuple(
        candidate if item.relative_path == document.relative_path else item
        for item in corpus.request_documents
    )
    return Corpus(
        corpus.root,
        corpus.modules,
...
def _validate_candidate(
    ...
    if blocking:
        raise RequestLifecycleError(
            "candidate request violates the evidence corpus:\n" + "\n".join(blocking)
        )
    return candidate
...
def _check_expected(
    ...
    if current_status != expected_status:
        raise RequestLifecycleError(
            f"stale request state: expected {expected_status!r}, "
    ...
    if not SHA256_RE.fullmatch(expected_sha256):
        raise RequestLifecycleError(
            "expected document SHA-256 must be 64 lowercase hexadecimal characters"
        )
    if document.sha256 != expected_sha256:
        raise RequestLifecycleError(
            f"stale request document: expected sha256 {expected_sha256}, "
            f"current sha256 is {document.sha256}"
        )
...
def _history_event(
    *,
    ...
    if actor_pr is not None and (
        isinstance(actor_pr, bool) or actor_pr <= 0
    ):
...
def transition_value(
    ...
    *,
    ...
    if expected_status not in REQUEST_STATUSES or to_status not in REQUEST_STATUSES:
        raise RequestLifecycleError(
            "expected and target statuses must use the canonical request status enum"
        )
    if to_status in {"result-available", "consumed"}:
        raise RequestLifecycleError(
            f"use the dedicated command for transition to {to_status}"
        )
    current = request.get("status")
    if current != expected_status:
        raise RequestLifecycleError(
            f"stale request state: expected {expected_status!r}, "
            f"current status is {current!r}"
        )
    if to_status not in REQUEST_TRANSITIONS.get(expected_status, frozenset()):
        raise RequestLifecycleError(
            f"invalid request transition {expected_status!r} -> {to_status!r}"
        )
...
def record_result_value(
    ...
    *,
    ...
    if expected_status != "active" or request.get("status") != "active":
        raise RequestLifecycleError(
            "owner result may be recorded only from expected active status"
        )
    ...
    normalized_refs, _ = _stable_refs(result_refs, "result refs")
    normalized_proves = _unique_nonempty(proves, "proves")
    normalized_nonproof = _unique_nonempty(
        does_not_prove, "does_not_prove"
    )
    normalized_blockers = (
        [] if not blockers else _unique_nonempty(blockers, "blockers")
    )
    parse_stable_result_ref(owner_evidence_ref)
...
def _source_matches_ref(
    source: Mapping[str, Any], reference: StableResultRef
) -> bool:
...
def consume_result_value(
    ...
    *,
    ...
    if (
        ...
        or request.get("status") != "result-available"
    ):
        raise RequestLifecycleError(
            "owner result may be consumed only from expected result-available status"
        )
    if actor_role not in {"collector", "reviewer", "automation"}:
        raise RequestLifecycleError(
            "result consumption must be recorded by collector, reviewer or automation"
        )
    ...
    allowed_source_types = OWNER_RESULT_SOURCE_TYPES.get(
        owner_kind, frozenset()
    )
    documents = {
        str(document.value.get("evidence_id")): document
    ...
    for evidence_id in normalized_ids:
        ...
        if document is None:
            raise RequestLifecycleError(
                f"consuming evidence record {evidence_id!r} does not exist"
            )
        record = document.value
        if request.get("request_id") not in record.get(
            "owner_request_refs", []
        ):
            raise RequestLifecycleError(
                f"evidence record {evidence_id} must link owner request "
                f"{request.get('request_id')}"
            )
        record_level = record.get("proof_level")
        if (
            record_level not in PROOF_RANK
            or PROOF_RANK[record_level] > PROOF_RANK[result_level]
        ):
            raise RequestLifecycleError(
                f"evidence record {evidence_id} proof {record_level!r} "
                f"exceeds owner result proof {result_level!r}"
            )
        ...
        for source in record.get("sources", []):
            if (
                ...
                or source.get("source_type") not in allowed_source_types
            ):
                continue
            source_level = source.get("proof_level_reached")
            if (
                source_level not in PROOF_RANK
                or PROOF_RANK[source_level] > PROOF_RANK[result_level]
            ):
                raise RequestLifecycleError(
                    f"evidence source in {evidence_id} exceeds owner result "
                    f"proof {result_level!r}"
                )
            if any(
        ...
        if not matching:
            raise RequestLifecycleError(
                f"evidence record {evidence_id} lacks a {owner_kind} "
                "owner-result source matching a stable result reference"
            )
...
def _restore_files(
    backups: Mapping[Path, bytes | None], root: Path
) -> None:
...
def apply_candidate(
    *,
    ...
    if baseline_errors:
        raise RequestLifecycleError(
            "current evidence corpus is invalid:\n"
            + "\n".join(baseline_errors)
        )
    document = _request_document(corpus, request_id)
    _check_expected(
        document, expected_status, expected_document_sha256
    )
    candidate_value = mutation(copy.deepcopy(document.value), corpus)
    candidate = _validate_candidate(
        corpus, document, candidate_value, as_of
    )
    if not write:
    ...
    try:
        ...
        if final_errors:
            raise RequestLifecycleError(
                "post-write corpus validation failed:\n"
                + "\n".join(final_errors)
            )
    except Exception:
...
def parser() -> argparse.ArgumentParser:
    ...
    transition = commands.add_parser(
        "transition", help="advance one non-result request state"
    )
    common(transition)
    transition.add_argument(
        "--to-status", required=True, choices=sorted(REQUEST_STATUSES)
    )
    transition.add_argument(
        "--actor-role",
        required=True,
        choices=("collector", "owner", "reviewer", "automation"),
    )
    ...
    result = commands.add_parser(
        "record-result", help="record an owner-produced stable result"
    )
    common(result)
    ...
    result.add_argument(
        "--proof-level", required=True, choices=tuple(PROOF_RANK)
    )
    result.add_argument("--proves", action="append", required=True)
    result.add_argument(
        "--does-not-prove", action="append", required=True
    )
    result.add_argument("--blocker", action="append", default=[])

    consume = commands.add_parser(
        "consume-result", help="consume a stable owner result into linked evidence"
    )
    common(consume)
    consume.add_argument(
        "--actor-role",
        required=True,
        choices=("collector", "reviewer", "automation"),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "transition":
            def mutation(request: dict[str, Any], corpus: Corpus) -> dict[str, Any]:
                del corpus
