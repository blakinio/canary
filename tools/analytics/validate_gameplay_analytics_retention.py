...
MIGRATION = ROOT / "schema/gameplay_analytics_migrations/004_daily_retention.sql"
...
def validate_migration(text: str) -> None:
    for table in ("analytics_daily_balance", "analytics_maintenance_state"):
        require(f"CREATE TABLE IF NOT EXISTS `{table}`" in text, f"retention migration lacks {table}")
    for column in (
        "session_date",
        "server_version",
        "hunt_area",
    ...
    require("analytics_sessions_started_id" in text, "retention migration lacks raw-session cleanup index")
    require("IF NOT EXISTS" in text, "retention migration must be repeatable")
    require("ENGINE=InnoDB" in text, "retention tables must use InnoDB")
