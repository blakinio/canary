...
if [[ "${schema_version}" -lt "${REQUIRED_SCHEMA_VERSION}" ]]; then
	echo "Gameplay Analytics schema version ${schema_version} is older than required maintenance version ${REQUIRED_SCHEMA_VERSION}" >&2
	exit 1
fi
...
processed_days=0
while [[ -n "${current_date}" && "${current_date}" <="${target_date}" && "${processed_days}" -lt "${MAX_DAYS_PER_RUN}" ]]; do
